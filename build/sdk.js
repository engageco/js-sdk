...
*/
var gibberishaes, event_dispatcher_EventDispatcher, jquery, cryptosha1, engage_sdk_services_BaseRESTService, event_dispatcher_Event, engage_sdk_services_KeepAliveService, engage_sdk_utils_PresenceMonitor, engage_sdk_utils_UserPageTracker, engage_sdk_services_ServiceQueue, engage_sdk_services_GetUsersService, headjs, jquerycookie, EngageSDK;
  ...
  var onXMPPSuccess = function (data, textStatus, jqXHR) {
    ...
    var presenceResponses = jQuery(data).find('presence').each(function (index, item) {
      ...
      if (oldStatus != self.servers[server][user].status) {
        onStatusChange.apply(self, [
          jid,
          self.servers[server][user].status,
          self.servers[server][user].callback
        ]);
      }
    });
  };
...
EngageSDK = function (require, jQuery, PresenceMonitor, UserPageTracker, BaseRESTService, ServiceQueue, GetUsersService, Event) {
