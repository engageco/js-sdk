...
*/
var gibberishaes, event_dispatcher_EventDispatcher, jquery, cryptosha1, engage_sdk_services_BaseRESTService, event_dispatcher_Event, engage_sdk_services_KeepAliveService, engage_sdk_services_TrackService, jquerycookie, engage_sdk_managers_TrackingManager, engage_sdk_utils_PresenceMonitor, engage_sdk_utils_UserPageTracker, engage_sdk_services_ServiceQueue, engage_sdk_services_GetUsersService, engage_sdk_services_GetWidgetConfigService, headjs, EngageSDK;
(function (root, factory) {
  if (typeof exports === 'object') {
    // Node. 
    module.exports = factory();
  } else if (true) {
    gibberishaes = function () {
  ...
  var Nr = 14,
    ...
    Nk = 8, Decrypt = false, enc_utf8 = function (s) {
      try {
    ...
    GEX = Gx(14), enc = function (string, pass, binary) {
      // string, password in plaintext
      var salt = randArr(8), pbe = openSSLKey(s2a(pass, binary), salt), key = pbe.key, iv = pbe.iv, cipherBlocks, saltBlock = [[
            83,
            97,
            108,
            116,
      ...
      function convertToWordArray(numArr) {
        var lWordCount, lMessageLength = numArr.length, lNumberOfWords_temp1 = lMessageLength + 8, lNumberOfWords_temp2 = (lNumberOfWords_temp1 - lNumberOfWords_temp1 % 64) / 64, lNumberOfWords = (lNumberOfWords_temp2 + 1) * 16, lWordArray = [], lBytePosition = 0, lByteCount = 0;
      ...
      var _chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/', chars = _chars.split(''), encode = function (b, withBreaks) {
          var flatArr = [], b64 = '', i, broken_b64, totalChunks = Math.floor(b.length * 16 / 3);
...
event_dispatcher_EventDispatcher = function () {
...
cryptosha1 = undefined;
engage_sdk_services_BaseRESTService = function (jQuery, EventDispatcher, GibberishAES) {
  ...
  BaseRESTService.prototype.packageRequestData = function (paramsRef) {
    ...
    if (this.useToken && BaseRESTService.token != null) {
      params.unshift({
        name: 'requestId',
        value: BaseRESTService.requestId
      });
      params.unshift({
        name: 'token',
        value: BaseRESTService.token
      });
    }
    params.unshift({
      name: 'apiKey',
...
}(jquery, event_dispatcher_EventDispatcher, gibberishaes);
event_dispatcher_Event = function () {
...
engage_sdk_services_KeepAliveService = function (jQuery, BaseRESTService, Event) {
...
}(jquery, engage_sdk_services_BaseRESTService, event_dispatcher_Event);
engage_sdk_services_TrackService = function (jQuery, BaseRESTService, Event) {
...
engage_sdk_managers_TrackingManager = function (jQuery, TrackService) {
  ...
  TrackingManager.translateChatSource = function (from) {
    ...
    case 0:
      return 'Profile_page';
    //profile page (profile.engage.com/userdomain)
    case 1:
      return 'Status_button';
    //in-page status buttons
    case 2:
      return 'User_card';
    //usercard (hover card)
    case 3:
      return 'Email_sig';
    //image signature (email)
    case 4:
      return 'HTML_profile';
    //HTML profile embed
    case 5:
      return 'HTML_listing';
    //toolbar/lightbox top level directory listing
    case 6:
      return 'Chat_Bubble';
    //chat bubble "Reconnect"
    case 7:
  ...
  TrackingManager.prototype.trackEvent = function (type, description) {
    if (this.applicationName == null || this.companyId == null)
      return false;
    var trackService = new TrackService(this.applicationName, this.customerDomain, this.publisherDomain, this.companyId, this.userGuid, this.tag, type, description);
    trackService.execute();
  };
  return TrackingManager;
}(jquery, engage_sdk_services_TrackService);
engage_sdk_utils_PresenceMonitor = function (jQuery, EventDispatcher, Event) {
  ...
  var onStatusSuccess = function (data, textStatus, jqXHR) {
    ...
    for (var user in data) {
      ...
      if (oldStatus != this.servers[server][user].status) {
        onStatusChange.apply(this, [
          user,
          this.servers[server][user].status,
          this.servers[server][user].callback
        ]);
      }
    }
  };
...
}(jquery, event_dispatcher_EventDispatcher, event_dispatcher_Event);
engage_sdk_utils_UserPageTracker = function (jQuery) {
  var config = {
  ...
  var onUpdatePageViewport = function (event) {
    ...
    this.currentPageData.viewport.width = jQuery(window).width();
    //body.width();
    this.currentPageData.viewport.height = jQuery(window).height();
    //body.height();
...
engage_sdk_services_ServiceQueue = function (jQuery, KeepAliveService, EventDispatcher, Event) {
  var TOKEN_WINDOW = 5;
  ...
  return ServiceQueue;
}(jquery, engage_sdk_services_KeepAliveService, event_dispatcher_EventDispatcher, event_dispatcher_Event);
engage_sdk_services_GetUsersService = function (jQuery, BaseRESTService, Event) {
...
engage_sdk_services_GetWidgetConfigService = function (jQuery, BaseRESTService, Event) {
...
(function (win, undefined) {
  ...
  var ua = nav.userAgent.toLowerCase(), mobile = /mobile|android|kindle|silk|midp|phone|(windows .+arm|touch)/.test(ua);
...
(function (win, undefined) {
  ...
  var doc = win.document,
    /*
            ...
            */
    /* CSS modernizer */
    el = doc.createElement('i'), style = el.style, prefs = ' -o- -moz- -ms- -webkit- -khtml- '.split(' '), domPrefs = 'Webkit Moz O ms Khtml'.split(' '), headVar = win.head_conf && win.head_conf.head || 'head', api = win[headVar];
  // Thanks Paul Irish!
...
(function (win, undefined) {
  'use strict';
  //#region variables
  var doc = win.document, domWaiters = [], handlers = {},
    // user functions waiting for events
    assets = {},
    // loadable items in various states
    isAsync = 'async' in doc.createElement('script') || 'MozAppearance' in doc.documentElement.style || win.opera, isDomReady,
    /*** public API ***/
  ...
  ******************************************************/
  function loadAsset(asset, callback) {
    ...
    function error(event) {
      ...
      // do callback
      callback();  // need some more detailed error handling here
    }
...
headjs = undefined;
EngageSDK = function (require, jQuery, TrackingManager, PresenceMonitor, UserPageTracker, BaseRESTService, ServiceQueue, GetUsersService, GetWidgetConfigService, Event) {
  ...
  EngageSDK.prototype.drawWidget = function (config, callback) {
    ...
    case 'toolbar':
      ...
      head.load([
        ...
        sdk.widget = new EngageToolbar(sdk, config.options, callback);  //callback(sdk.widget);
      });
      break;
    case 'custom':
    default:
  ...
  return EngageSDK;
}({}, jquery, engage_sdk_managers_TrackingManager, engage_sdk_utils_PresenceMonitor, engage_sdk_utils_UserPageTracker, engage_sdk_services_BaseRESTService, engage_sdk_services_ServiceQueue, engage_sdk_services_GetUsersService, engage_sdk_services_GetWidgetConfigService, event_dispatcher_Event);if(typeof define === 'function' && define.amd ) {define('EngageSDK', [], function() {return EngageSDK;});}jQuery.noConflict(true);return EngageSDK;}));